    ...
    <Card className="w-full max-w-md mx-auto shadow-lg border-2 border-purple-200">
          ...
          <div className="flex flex-col gap-3">
            {messages.map((message) => (
              ...
              >
                ...
                <div className="whitespace-pre-wrap break-words overflow-wrap-anywhere overflow-hidden max-w-full">{message.content}</div>
              </div>
            ))}
            {isTyping && (
      ...
      <CardFooter className="p-4 pt-2 border-t">
        <form
          ...
          className="flex w-full gap-2 items-center flex-shrink-0 flex-nowrap"
        >
          <Input
            placeholder="Ask Maggie a question or request a story..."
            value={input}
            onChange={(e) => setInput(e.target.value)}
            className="flex-1 min-w-0 h-10 text-base border-indigo-200 focus-visible:ring-indigo-400"
          />
          <Button 
            type="submit" 
            size="icon" 
            disabled={!input.trim() || isTyping}
            className="h-10 w-10 flex-shrink-0 flex-none bg-indigo-600 hover:bg-indigo-700"
          >
